const CACHE = 'game-allowance-v8';
const BG = './a_bright_crisp_photorealistic_voxel_art_minecraf.png';
const ASSETS = ['./', './index.html', './manifest.webmanifest', './icon.svg', './daily-limit.js', './gymnastics.js', BG];
...
self.addEventListener('fetch', event => {
 ...
 if(isAppPage){event.respondWith(fetch(event.request).then(r=>r.text()).then(html=>{
   html=html.replace(/<script src="\.\/axis-charts\.js[^\"]*"><\/script>/g,'');
   html=html.replace(/<script src="\.\/switch-setting\.js[^\"]*"><\/script>/g,'');
   ...
   html=html.replace('</body>','<script src="./switch-setting.js?v=2"></script></body>');
 ...
 if(url.pathname.endsWith('/switch-setting.js')){event.respondWith(fetch(event.request,{cache:'no-store'}));return;}
