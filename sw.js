const CACHE = 'game-allowance-v7';
const BG = './a_bright_crisp_photorealistic_voxel_art_minecraf.png';
const ASSETS = ['./', './index.html', './manifest.webmanifest', './icon.svg', './daily-limit.js', './gymnastics.js', './switch-setting.js', BG];
...
self.addEventListener('fetch', event => {
 ...
 if(isAppPage){event.respondWith(fetch(event.request).then(r=>r.text()).then(html=>{
   ...
   if(!html.includes('switch-setting.js'))html=html.replace('</body>','<script src="./switch-setting.js?v=1"></script></body>');
