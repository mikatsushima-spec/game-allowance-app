...
function dailyGameUsed(child,dateStr){return entries.filter(e=>e.child===child&&e.date===dateStr&&e.reward==='game'&&Number(e.delta)<0).reduce((sum,e)=>sum+Math.abs(Number(e.delta)||0),0);}
...
renderDashboard=function(){
  ...
  document.querySelectorAll('#balances .balance-card').forEach((card,i)=>{
    ...
    box.innerHTML=`<div style="font-size:10px;font-weight:800;color:#64748b">今日のゲーム ${limit===240?'休日':'平日'}</div><div style="margin-top:2px;font-size:13px;font-weight:900;color:${over?'#b42318':'#0f172a'}">${fmtMin(used)} / ${fmtMin(limit)}</div><div style="height:6px;margin-top:7px;border-radius:999px;background:#e8eef6;overflow:hidden"><div style="height:100%;width:${pct}%;border-radius:999px;background:${over?'#b42318':'#3567c8'}"></div></div>`;
